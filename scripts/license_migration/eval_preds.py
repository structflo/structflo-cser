...
from structflo.cser.inference.metrics import ImageEval, evaluate, fitness, load_yolo_labels
...
def load_images(preds_path: Path, labels_dir: Path) -> list[ImageEval]:
    ...
    for stem, rec in d["images"].items():
        gt_boxes, gt_cls = load_yolo_labels(labels_dir / f"{stem}.txt", rec["width"], rec["height"])
        out.append(
            ImageEval(
                pred_boxes=np.array([x["bbox"] for x in rec["dets"]], dtype=np.float64).reshape(-1, 4),
                pred_scores=np.array([x["conf"] for x in rec["dets"]], dtype=np.float64),
                pred_classes=np.array([x["class_id"] for x in rec["dets"]], dtype=np.int64),
                gt_boxes=gt_boxes,
                gt_classes=gt_cls,
            )
        )
...
def coco_crosscheck(images: list[ImageEval]) -> dict:
    ...
    gt = {"images": [], "annotations": [], "categories": [{"id": c, "name": n} for c, n in NAMES.items()]}
    dt = []
    aid = 1
    for i, im in enumerate(images):
        ...
        for b, c in zip(im.gt_boxes, im.gt_classes):
            w, h = b[2] - b[0], b[3] - b[1]
            gt["annotations"].append({"id": aid, "image_id": i, "category_id": int(c), "bbox": [b[0], b[1], w, h], "area": w * h, "iscrowd": 0})
            aid += 1
        for b, s, c in zip(im.pred_boxes, im.pred_scores, im.pred_classes):
            dt.append({"image_id": i, "category_id": int(c), "bbox": [b[0], b[1], b[2] - b[0], b[3] - b[1]], "score": float(s)})
    coco = COCO()
    coco.dataset = gt
    coco.createIndex()
...
def main() -> None:
    ...
    print(f"{args.preds}  ({len(images)} images; backend={meta.get('backend')} imgsz={meta.get('imgsz')})")
    print(f"  {'class':10s} {'n_gt':>5s} {'P@'+str(args.conf):>8s} {'R@'+str(args.conf):>8s} {'FP':>5s} {'mAP50':>7s} {'mAP50-95':>9s}")
    for c, n in NAMES.items():
        r = res[c]
        print(f"  {n:10s} {r.n_gt:5d} {r.precision:8.3f} {r.recall:8.3f} {r.fp:5d} {r.ap50:7.4f} {r.ap:9.4f}")
    a = res["all"]
    print(f"  {'all':10s} {'':5s} {a['precision']:8.3f} {a['recall']:8.3f} {'':5s} {a['mAP50']:7.4f} {a['mAP50-95']:9.4f}   fitness {fitness(a):.4f}")
    payload = {
        "preds": str(args.preds), "meta": meta, "conf": args.conf, "n_images": len(images),
        "all": a, "fitness": fitness(a),
        "per_class": {n: {"n_gt": res[c].n_gt, "P": res[c].precision, "R": res[c].recall, "TP": res[c].tp, "FP": res[c].fp, "mAP50": res[c].ap50, "mAP50-95": res[c].ap} for c, n in NAMES.items()},
    }
    if args.cocoeval:
        cc = coco_crosscheck(images)
        print("  pycocotools cross-check:", {NAMES[c]: {"AP50": round(v[0], 4), "AP50-95": round(v[1], 4)} for c, v in cc.items()})
        payload["pycocotools"] = {NAMES[c]: {"mAP50": v[0], "mAP50-95": v[1]} for c, v in cc.items()}
    if args.out:
