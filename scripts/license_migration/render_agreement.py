...
def _dets(model, im: Image.Image, conf: float, imgsz: int) -> list[tuple]:
    ...
    return [(box.xyxy[0].cpu().numpy(), float(box.conf[0]), int(box.cls[0])) for box in res.boxes]
...
def main() -> None:
    ...
    ap.add_argument("--pdf-dir", type=Path, default=Path("data/cser_staging_all/staging_pdfs"))
    ...
    ap.add_argument("--out", type=Path, default=Path("runs/license_migration/eval/render_agreement.json"))
    args = ap.parse_args()
    ...
    for dpi in args.dpis:
        ...
        summary[dpi] = {
            "pages": n_pages, "size_mismatch_pages": size_mismatch, "dets_fitz": tot_a, "dets_pdfium": tot_b,
            "matched": tot_m, "agreement_f1": f1, "pages_same_count": same_count,
        ...
        print(f"[{dpi} dpi] pages {n_pages} (size mismatch {size_mismatch}); dets fitz {tot_a} / pdfium {tot_b}; "
              f"matched {tot_m} → agreement F1 {f1:.4f}; same-count pages {same_count}/{n_pages}; "
              f"median |Δconf| {summary[dpi]['median_abs_conf_diff']}; pixel-diff frac {summary[dpi]['mean_pixel_frac_diff_gt32']}", flush=True)
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(json.dumps({"weights": args.weights, "conf": args.conf, "imgsz": args.imgsz, "n_pdfs": len(pdfs), "by_dpi": summary}, indent=2))
