...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=float,
        default=1.0,
        help="pre-downscale factor (0.48 ≈ 144 dpi, 0.5 ≈ 150 dpi); boxes mapped back",
    )
    args = ap.parse_args()
    ...
    for split in args.splits:
        paths = sorted(
            p
            for p in Path(SPLITS[split]).iterdir()
        ...
        for i, p in enumerate(paths):
            ...
            dets = det.predict(
                arr, conf=args.conf, imgsz=args.imgsz, max_det=args.max_det
            )
        ...
        print(
            f"[{split}] wrote {len(paths)} images, median {med:.1f} ms/img → {out_dir / f'{split}.json'}",
            flush=True,
        )
