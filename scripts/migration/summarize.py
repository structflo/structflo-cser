...
def e2e_row(tag: str, split: str, suffix: str = "") -> str | None:
    ...
    m = d["matchers"]
    cells = " | ".join(
        f"{m[k]['P']:.3f} / {m[k]['R']:.3f} / **{m[k]['F1']:.3f}**"
        for k in ("Hungarian", "LPS", "Relational")
    )
...
def main() -> None:
    ...
    ap.add_argument(
        "--tags", nargs="*", default=["yolo_v0.4", "dfine_l_synth", "dfine_l_plus"]
    )
    ...
    for tag in args.tags:
        for split in ("real_test", "real_val", "synth_test"):
            ...
            if r:
                lines.append(r)
        for c in ("0.35", "0.4", "0.45", "0.5"):
            for split in ("real_test", "real_val"):
                r = det_row(tag, split, f"_conf{c}")
                if r:
                    lines.append(
                        r.replace(f"| {tag}_conf{c} |", f"| {tag} @ conf {c} |")
                    )
        for sc in ("0.48", "0.5"):
            for split in ("real_test", "real_val"):
    ...
    for tag in args.tags:
        for split in ("test", "val"):
            for suffix in (
                ...
                if r:
                    label = suffix.replace("_conf", " @ conf ").replace("_scale", " @ ")
                    lines.append(r.replace(f"| {tag}{suffix} |", f"| {tag}{label} |"))
            for suffix, label in (
                ("_relnew", "recalibrated relational @0.3"),
                ("_relnew_c0.5", "recalibrated relational @0.5"),
            ):
                r = _load(f"{tag}_e2e_{split}{suffix}")
                if r:
                    m = r["matchers"]["Relational"]
                    lines.append(
                        f"| {tag} + {label} | {split} | — | — | {m['P']:.3f} / {m['R']:.3f} / **{m['F1']:.3f}** |"
                    )
    # conf sweep on val
    sweep = []
    for tag in args.tags:
        for c in ("0.1", "0.2", "0.25", "0.3", "0.35", "0.4", "0.5"):
            ...
            if r:
                m = r["matchers"]
                sweep.append(
                    f"| {tag} | {c} | {m['Hungarian']['F1']:.3f} | {m['LPS']['F1']:.3f} | {m['Relational']['F1']:.3f} |"
                )
    if sweep:
        lines += [
            "",
    ...
    for m_ in ("0.0", "1.0", "2.0", "3.0", "4.0"):
        ...
        if r:
            ms.append(
                f"| {m_} | {r['matchers']['Relational']['P']:.3f} | {r['matchers']['Relational']['R']:.3f} | {r['matchers']['Relational']['F1']:.3f} |"
            )
    if ms:
        lines += [
            "",
    ...
    if ra:
        ...
        for dpi, v in ra["by_dpi"].items():
            lines.append(
                f"| {dpi} | {v['pages']} | {v['agreement_f1']:.4f} | {v['pages_same_count']} | {v['median_abs_conf_diff']:.3f} |"
            )
    text = "\n".join(lines)
    print(text)
    if args.out:
